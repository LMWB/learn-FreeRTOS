...
#define DO_NOTHING()	__NOP()			// asm("nop") same as __NOP()
...
#include "usart.h"
#define UART_MAX_DELAY_TIME								100
#define UART_TERMINAL_HANDLER 	            			huart2
#define UART_TERMINAL_INSTANCE 	            			USART2
#define UART_TERMINAL_SEND(string, size)    			HAL_UART_Transmit(&UART_TERMINAL_HANDLER, string, size, UART_MAX_DELAY_TIME)
...
#define UART_RS485_SEND(string, size)    				HAL_UART_Transmit(&UART_RS485_HANDLER, string, size, UART_MAX_DELAY_TIME)
